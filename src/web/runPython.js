...
import {
    INDEX_QUEUE,
    INDEX_WAITING,
    NUM_POSITIONS, STATUS_DATA, STATUS_DONE,
    STATUS_FAIL,
    STATUS_READY,
...
export default function runPyScript(key, script, args) {
    return new Promise((resolve) => {
        ...
        const commBuff = window.Atomics ? new window.SharedArrayBuffer(4 * NUM_POSITIONS) : null;
            ...
            worker.onmessage = async (e) => {
                if (e.data.out) {
                    ...
                    if (currIndex < currFile.content.length) {
                        const block = currFile.content.slice(currIndex, currIndex + maxStrLen);
                        currIndex += maxStrLen;
