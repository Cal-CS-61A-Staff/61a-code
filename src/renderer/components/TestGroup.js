...
export default class TestGroup extends React.Component {
    ...
    render() {
        ...
        const expandIconClass = expanded ? "fas fa-caret-down" : "fas fa-caret-right";
        ...
        return (
            <>
                <div
                    ...
                    style={{ paddingLeft: 10, ...(expanded ? { background: "slategray" } : {}) }}
                >
                    {" "}
                    <i className={expandIconClass} />
                    {"  "}
                    {expanded ? false : successDisplay}
                    {" "}
                    {this.props.name}
                </div>
                {children}
