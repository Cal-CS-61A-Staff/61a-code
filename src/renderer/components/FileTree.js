...
const NoCollapse = React.forwardRef(
    (props, ref) => <Collapse {...props} timeout={0} ref={ref} />,
);
...
export default function FileTree({ onFileSelect }) {
    ...
    const handleToggle = (nodeID, isExpanding) => {
        if (isExpanding) {
            setExpanded(currExpanded => [...new Set(currExpanded.concat([nodeID]))]);
        } else {
        ...
        <div className="FileTree">
            <ThemeProvider theme={theme}>
                <TreeView
                    ...
                    expanded={expanded}
                >
                    <TreeElement location="/" onToggle={handleToggle} onFileSelect={onFileSelect} />
                </TreeView>
            </ThemeProvider>
        </div>
    );
