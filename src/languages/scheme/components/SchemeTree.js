...
export default class SchemeTree extends React.PureComponent {
    ...
    displayTreeWorker(data, container, x, y, level, starts) {
        ...
        container.text(data.str).font("family", "Monaco, monospace").font("size", 14).dx(x)
            .dy(y);
        ...
        for (const child of data.children) {
            ...
            container.line(x + xDelta + parentLen / 2, y + charHeight + 5,
                Math.max(x + xDelta - 100000, starts[level + 1])
                + child.str.length * charWidth / 2 + 5,
                y + 60)
                .stroke({ width: 3, color: "#c8c8c8" }).back();
            this.displayTreeWorker(child, container,
                Math.max(x + xDelta - 100000, starts[level + 1]), y + 50, level + 1, starts);
