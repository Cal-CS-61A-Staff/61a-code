import {
    assert, placeHorizontally, tableFormat, generateHslaColors,
} from "./utils.js";
...
function join(tableNames, tableData, out) {
    ...
    for (const tableName of tableNames) {
        ...
        const tableToJoin = { columns: cols, values: tableData.get(realName).values };
    ...
    if (tableNames.length > 1) {
        ...
        for (const table of tables) {
            ...
            const duplicatedTable = {
                columns: table.columns, values: copyRows(table.values, duplicates, repeats),
            };
            duplicatedTables.push(duplicatedTable);
            duplicatedTableDivs.push(tableFormat(
                duplicatedTable,
...
function where(table, whereClause, selectClause, out) {
    ...
    const highlightRows = (i => (selectedRows.includes(i) ? "lightgreen" : "white"));
...
function evaluate(whereClause, columnNames, rowValues, selectClause, allRows) {
    if (whereClause.type === "atom") {
        ...
        const left = evaluate(whereClause.left, columnNames, rowValues, selectClause, allRows);
        const right = evaluate(whereClause.right, columnNames, rowValues, selectClause, allRows);
