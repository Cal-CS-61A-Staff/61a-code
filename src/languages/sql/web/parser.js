...
function getGroups(buffer) {
    assert(buffer.popNext().toUpperCase() === "BY", "GROUP must be followed by BY");
...
function getExpr(buffer) {
    ...
    for (;;) {
        ...
        if (buffer.getNext() === "(") {
            ...
            val = {
                type: "combination", operator, left: { type: "atom", val: { type: "numeric", val: "0" } }, right: getExpr(buffer),
            };
        } else {
            // grab single
            ...
            if (first === "\"" || first === "\"") {
                ...
                assert(buffer.popNext() === ")", "Aggregates should only take one expression.");
